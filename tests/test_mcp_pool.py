...
def test_pool_size_clamped():
    ...
    assert pool._resolve_size(SimpleNamespace(mcp_postgres_pool_size=100)) == 16
    assert pool._resolve_size(SimpleNamespace(mcp_postgres_pool_size=0)) == 1
    settings = Settings(_env_file=None, mcp_postgres_pool_size=8)
    assert pool._resolve_size(settings) == 8
