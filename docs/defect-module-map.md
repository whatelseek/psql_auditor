...
| CORE-006 | `src/auditor/application_runtime.py`, `src/auditor/task_registry.py`, `src/auditor/api/app.py`, `src/auditor/api/openai_compat.py`, `src/auditor/graph.py`, `src/auditor/config.py`, `src/auditor/results_store.py`, `src/auditor/tools/mcp_client.py`, `src/auditor/workflows/runner.py`, `src/auditor/runtime_target.py` | `ApplicationRuntime`, `TaskRegistry`, `create_app`, `runtime_from_request`, `build_mcp_tools`, `bind_results_store`, `bind_app_settings`, `acquire_run_checkpointer`, `release_run_resources` | RESOLVED | Each FastAPI app owns settings/graph/MCP pool/results store/task registry/scoped leases via lifespan; production handlers use `request.app.state.runtime`; no production `_graph`/`_STORE`/`_POOL`; deprecated getters remain test-only; tests: `tests/test_application_runtime.py`. FLOW-007 still tracks removing compatibility getters. |
