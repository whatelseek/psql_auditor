...
| Field | Value |
|-------|-------|
| Reviewed commit SHA | `00cb8bab3e5c1d22f8e3199efcddfc6f08bc4e6e` |
...
| CORE-001 | `src/auditor/client_registry.py`, `src/auditor/legacy_compat.py`, `src/auditor/evidence_store.py`, `src/auditor/results_store.py`, `src/auditor/audit_registry.py`, `src/auditor/workflows/intake.py`, `src/auditor/workflows/runner.py`, `src/auditor/workflows/multi_runner.py` | `require_client_id`, `require_audit_run_id`, `assert_client_owns_run`, `ClientOwnershipError`, `ResultsStore.start_session` | RESOLVED | Both identifiers required on create/persist; warehouse rejects empty client_id; run→client reassignment and conflicting resume ownership rejected; AUD-003 fixtures reused in `tests/test_client_audit_run_identity.py`. Legacy `run_id` = evidence folder only. Checkpoint isolation remains CORE-005. |
