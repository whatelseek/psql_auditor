...
| Field | Value |
|-------|-------|
| Reviewed commit SHA | `4094d76b54d592d2b4334323dcc260c0e32f6e59` |
...
| INPUT-001 | `src/auditor/domain/audit_request.py`, `src/auditor/domain/__init__.py`, `src/auditor/workflows/multi_runner.py`, `src/auditor/session_store.py`, `src/auditor/graph.py`, `src/auditor/workflows/protocols.py`, `src/auditor/api/openai_compat.py` | `AuditRequest`, `AuditRequestRejected`, `parse_audit_request`, `validate_audit_request_semantics`, `build_audit_request_from_selected_jobs`, `arun_request`, `start_frameworks_after_intake` | PARTIALLY IMPLEMENTED | Strict immutable v1 `AuditRequest` with typed rejection; intake and `arun_request` create jobs only after validation; NLP empty-scope fallback removed; secret-free persistence in `AuditRun.scope`/`meta.json`; credentials re-resolved from inventory. Tests: `tests/test_input001_audit_request.py`. Independent acceptance pending. |
